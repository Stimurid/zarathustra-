...
- **outer** — after each inner pass, `projection_step` runs; if `pending_diagnostic.mismatch`, `_invoke_reflective_epilogue` calls **S7 in reflective mode** (a dedicated call, distinct from the CONDITIONAL S7 inside the pass); the epilogue's delta MUST carry a `ReflectiveReturn`; `_apply_reflective_return` records it, marks the failing projection `PARTIAL`, revises `state.operation` (R1/R2) or `state.scene.telos` (R3), and sets `reentry_from` for the next pass.

**Why re-entry starts at `PHASE_ORDER[return_target_index + 1]`, not at `return_target`**: the return_target phase was itself the SITE of the revision — the ReflectiveReturn already wrote the new operation/scene into state. Re-running that phase with its original hint or an unrevised prompt would clobber the revision. This is ADR §14 *"changed-forward-action requirement"* made literal.
